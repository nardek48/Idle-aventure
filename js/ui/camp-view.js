...
function buildCampHTML() {
  ...
  if (offering) {
    ...
    h += '</div>';
    h += '<button class="settings-btn primary camp-embers-btn" type="button"' + (offering.canOffer ? ' onclick="StoryQuestManager.offerToEmbers(\'' + esc(offering.chapterId) + '\');"' : ' disabled') + '><img class=ico-inline src=images/Icons/camp/campfire.png> Offrir aux braises</button>';
    h += '</div>';
  }
