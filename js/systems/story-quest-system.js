...
var StoryQuestManager = {
  ...
  _ensureChapter: function (chapterId) {
    ...
    // v3.297.0 (W-1a) : choix pesants de ce chapitre, notés une fois pour toutes (registre des axes)
    if (!st.choices || typeof st.choices !== "object") st.choices = {};
  ...
  /* v3.297.0 (W-1a) — CHAPITRES ENCHAÎNÉS. Un chapitre qui déclare requiresChapter reste
     fermé tant que ce chapitre-là n'est pas terminé : pas d'étape au tableau, pas de suivi.
     Sans ce verrou, le chapitre du Désert s'afficherait en parallèle de la Forêt. */
  isChapterOpen: function (chapterId) {
    var chapter = STORY_QUESTS[chapterId];
    if (!chapter) return false;
    if (!chapter.requiresChapter) return true;
    return this.isChapterCompleted(chapter.requiresChapter);
  },

  /* Chapitres ouverts et pas encore terminés : ceux dont une étape peut être en cours. */
  activeChapterIds: function () {
    var self = this;
    return Object.keys(STORY_QUESTS).filter(function (id) {
      return self.isChapterOpen(id) && !self.isChapterCompleted(id);
    });
  },

  /* Une étape (id) est-elle atteinte, quel que soit son chapitre ? Chapitre terminé ou
     court-circuité : oui. Chapitre fermé : non. Id inconnu : oui (pas de verrou fantôme).
     Remplace les comparaisons d'index écrites en dur sur la Forêt (quêtes d'élite, secteurs). */
  isStepReached: function (stepId) {
    if (!stepId) return true;
    var ids = Object.keys(STORY_QUESTS);
    for (var i = 0; i < ids.length; i++) {
      var idx = STORY_QUESTS[ids[i]].steps.findIndex(function (s) { return s.id === stepId; });
      if (idx === -1) continue;
      var st = this.getState(ids[i]);
      if (st.skipped || this.isChapterCompleted(ids[i])) return true;
      if (!this.isChapterOpen(ids[i])) return false;
      return st.currentStep >= idx;
    }
    return true;
  },

  /* Choix pesants — notés dans le chapitre (storyQuests est sauvegardé et conservé à
     l'Ascension, aucun fichier protégé). Définitifs : un second appel ne change rien. */
  recordChoice: function (chapterId, key, value) {
    var st = this.getState(chapterId);
    if (!st || !key || st.choices[key] != null) return false;
    st.choices[key] = value;
  ...
  getChoice: function (key) {
    ...
    var ids = Object.keys(STORY_QUESTS);
    for (var i = 0; i < ids.length; i++) {
      var c = game.storyQuests[ids[i]].choices || {};
      if (c[key] != null) return c[key];
    }
    return null;
  },

  /* Registre des deux axes, calculé à partir des choix notés (STORY_CHOICE_AXES dit quel
     choix pèse sur quel pôle). Rien n'est stocké à part : un registre ne peut pas diverger. */
  getRegister: function () {
    var reg = { donner: 0, garder: 0, soi: 0, aeswyn: 0, chaos: 0 };
    var axes = window.STORY_CHOICE_AXES || {};
    var self = this;
    Object.keys(axes).forEach(function (key) {
      var v = self.getChoice(key);
      var poles = v != null && axes[key][v];
      (poles || []).forEach(function (pole) { if (reg[pole] != null) reg[pole] += 1; });
    });
    return reg;
  },
  ...
  getCurrentStep: function (chapterId) {
    ...
    var st = this.getState(chapterId);
    if (!chapter || st.skipped || !this.isChapterOpen(chapterId)) return null;
  ...
  _trackKills: function () {
    ...
    // v3.293.0 : plus de suivi de position (farm libre retiré).
    // v3.297.0 (W-1a) : chaque chapitre tient son propre repère ; seuls les chapitres actifs comptent.
    ...
    Object.keys(STORY_QUESTS).forEach(function (chapterId) {
      ...
      if (st.skipped || !self.isChapterOpen(chapterId) || self.isChapterCompleted(chapterId)) return;
      self._countKills(st, delta);
    });
  },

  /* Compteurs d'un chapitre actif pour `delta` victoires récentes. */
  _countKills: function (st, delta) {
    /* v3.268.0 (L-2) : combats gagnés avec un compagnon présent (étape forest_wenna).
       Compté AVANT les filtres de contexte : ce qu'on découvre est le compagnon, pas un
       lieu — tout combat de quête compte pareil (v3.293.0 : plus de farm libre). */
  ...
  runRetroactiveCheck: function () {
    ...
    Object.keys(game.storyQuests).forEach(function (id) { // v3.297.0 : chaque chapitre
      if (game.storyQuests[id]) game.storyQuests[id].lastSeenTotalKills = Number(game.totalKills || 0);
    });
    this._checkNow(true);
  },
  ...
  getOfferingInfo: function (chapterId) {
    ...
    return { chapterId: chapterId, step: step, items: items, canOffer: canOffer };
  },
