...
var SceneEngine = {
  ...
  buildCard: function (template, randomValues, slotWeightsOverride) {
    ...
    /* v3.300.0 (W-2) : parcours SCRIPTÉ (template.fixedCard) — chaque palier est écrit dans
       le canevas, rien n'est tiré. Pour les runs qui racontent une chose précise (la traversée).
       Copie profonde : le run persiste sa carte, le canevas partagé ne doit jamais bouger. */
    if (Array.isArray(template.fixedCard)) {
      return template.fixedCard.map(function (level) {
        return level.map(function (slot) {
          var s = JSON.parse(JSON.stringify(slot));
          if (s.type === "obstacle" && typeof s.riskMod !== "number") s.riskMod = 1;
          return s;
        });
      });
    }
